...
export const EndpointNav: React.FC<Props> = ({ apiId, def }) => {
  ...
  if (method && displayName) {
    ...
    if (displayName.toLowerCase().startsWith(methodLower)) {
      ...
      if (
        displayName.substring(methodLower.length, methodLower.length + 1) ===
        "-"
      ) {
    ...
    <SidebarMenuSubItem>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link
            to="/api/$apiId/endpoint/$endpointId"
            params={{ apiId, endpointId: path }}
            search={{ method }}
          >
        ...
        <TooltipContent side="right">
          <div className="flex flex-col">
            ...
            {def.summary && (
              <span className="text-xs opacity-80">{def.summary}</span>
            )}
          </div>
        </TooltipContent>
      </Tooltip>
