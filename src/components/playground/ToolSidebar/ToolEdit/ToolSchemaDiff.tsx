...
export const ToolSchemaDiff: React.FC<ToolSchemaDiffProps> = ({
  ...
  const descriptionChanged =
    modifiedDescription !== (originalTool.description || "");
  const schemaChanged =
    ...
    <div className="h-full space-y-6 overflow-y-auto pr-2">
      ...
      {nameChanged && (
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 dark:bg-red-500/5">
              <div className="mb-2 flex items-center gap-2">
                <Badge variant="destructive" className="text-xs">
                  Original
                </Badge>
              </div>
              <code className="font-mono text-sm text-red-700 dark:text-red-400">
                {originalTool.name}
              </code>
            </div>
            <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-3 dark:bg-green-500/5">
              <div className="mb-2 flex items-center gap-2">
                <Badge className="bg-green-600 text-xs hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600">
                  Modified
                </Badge>
              </div>
          ...
          <div className="space-y-3">
            <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 dark:bg-red-500/5">
              ...
              <p className="text-sm text-red-700 dark:text-red-400">
                {originalTool.description || (
                  <em className="text-red-600 dark:text-red-500">
                    No description
                  </em>
                )}
              </p>
            ...
            <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-3 dark:bg-green-500/5">
              ...
              <p className="text-sm text-green-700 dark:text-green-400">
                {modifiedDescription || (
                  <em className="text-green-600 dark:text-green-500">
                    No description
                  </em>
                )}
              </p>
      ...
      {schemaChanged && (
        <div className="space-y-3">
          <h3 className="text-foreground text-sm font-medium">Input Schema</h3>
          <SchemaDiff
...
function findSchemaChanges(
  ...
  Object.keys(modifiedProps).forEach((modifiedPropName) => {
    const modifiedProp = modifiedProps[modifiedPropName] as JSONSchema7 & {
    ...
    if (
      originalName &&
      originalName !== modifiedPropName &&
      originalProps[originalName]
    ) {
      ...
      const originalProp = originalProps[originalName] as JSONSchema7 & {
        "x-original-name"?: string;
        "x-ui-disabled"?: boolean;
      };
      ...
      const originalDisabled =
        (originalProp as JSONSchema7 & { "x-ui-disabled"?: boolean })?.[
          "x-ui-disabled"
        ] || false;
      const modifiedDisabled =
        (modifiedProp as JSONSchema7 & { "x-ui-disabled"?: boolean })?.[
          "x-ui-disabled"
        ] || false;
      ...
      const originalDesc =
        (originalProp as JSONSchema7 & { description?: string })?.description ||
        "";
      const modifiedDesc =
        (modifiedProp as JSONSchema7 & { description?: string })?.description ||
        "";
      ...
      if (
        (originalProp as JSONSchema7).type === "object" &&
        (modifiedProp as JSONSchema7).type === "object"
      ) {
        const nestedChanges = findSchemaChanges(
          originalProp as JSONSchema7,
          modifiedProp as JSONSchema7,
          currentPath,
        );
      ...
      const structuralChanges = findStructuralChanges(
        originalProp as JSONSchema7,
        modifiedProp as JSONSchema7,
        currentPath,
      );
  ...
  allPropertyNames.forEach((propName) => {
    ...
    const originalProp = originalProps[propName] as JSONSchema7 & {
      "x-original-name"?: string;
      "x-ui-disabled"?: boolean;
    };
    const modifiedProp = modifiedProps[propName] as JSONSchema7 & {
      "x-original-name"?: string;
      "x-ui-disabled"?: boolean;
    };
...
function findStructuralChanges(
  original: JSONSchema7,
  modified: JSONSchema7,
  path: string[],
): SchemaChange[] {
  ...
  // Check type changes
  const originalType = Array.isArray(original.type)
    ? original.type.join(" | ")
    : original.type;
  const modifiedType = Array.isArray(modified.type)
    ? modified.type.join(" | ")
    : modified.type;
        ...
        <div className="text-muted-foreground flex items-center gap-1 text-xs">
          {change.path.map((segment, index) => (
            <span key={index} className="flex items-center gap-1">
              ...
              <code className={cn("font-mono", colors.text)}>{segment}</code>
            </span>
          ))}
        </div>
      </div>

      <div className={cn("text-sm", colors.text)}>{change.details}</div>
          ...
          <div className="mt-3 space-y-2">
            <div className="text-xs">
              ...
              <span className="ml-2 text-red-600 dark:text-red-500">
                {(change.original as JSONSchema7 & { description?: string })
            ...
            <div className="text-xs">
              ...
              <span className="ml-2 text-green-600 dark:text-green-500">
                {(change.modified as JSONSchema7 & { description?: string })
                  ?.description || <em>No description</em>}
              </span>
            </div>
          </div>
