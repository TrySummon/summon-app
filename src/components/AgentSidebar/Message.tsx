...
>(
  (
    ...
    const renderReasoningPart = (part: MessagePart, partIndex: number) => {
          ...
          {isShowing && (
            <div className="mt-2 rounded-lg pl-5.5">
              <div className="text-muted-foreground space-y-2 text-xs">
                <div>
                  <Markdown textSize="base" mentionData={mentionData}>
    ...
    const renderTextPart = (part: MessagePart, partIndex: number) => {
      return (
        <div
          key={partIndex}
    ...
    const renderUserMessage = () => {
      ...
      return (
        <div
          className={cn(
            "p-3",
            !isRunning && onUpdateMessage ? "cursor-pointer" : "cursor-default",
          )}
          onClick={handleEditMessage}
          title={!isRunning && onUpdateMessage ? "Click to edit" : undefined}
        >
    ...
    const renderActionBar = () => {
      ...
      const showStopButton = isLatestUserMessage && isRunning && onStop;
      ...
      const showRevertButton =
        onRevert && message.id && (!isRunning || !isLatestUserMessage);
    ...
    const getMessageContainerStyles = () => {
      return cn(
        "group flex flex-col",
        isUser
          ? "bg-sidebar-accent/20 rounded-lg border hover:bg-sidebar-accent/80 transition-colors duration-200"
