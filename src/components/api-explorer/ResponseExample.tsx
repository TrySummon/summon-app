...
export const ResponseExample: React.FC<ResponseExampleProps> = ({ responses }) => {
  // Find the first successful response (e.g., 200, 201)
  const successStatusCode = Object.keys(responses).find(code => code.startsWith('2'));
  ...
  React.useEffect(() => {
    ...
    if (successStatusCode) {
      ...
      if ('content' in response && response.content) {
        ...
        Object.entries(response.content).forEach(([contentType, mediaTypeObject]) => {
          // Single example
          ...
          if (mediaTypeObject.examples) {
            Object.entries(mediaTypeObject.examples).forEach(([exampleName, exampleObj]) => {
              let exampleValue;
              
              if ('value' in exampleObj) {
                exampleValue = exampleObj.value;
              } else if ('$ref' in exampleObj && exampleObj.$ref) {
                // Handle references if needed
                // This would require the full OpenAPI spec to resolve
                // For now, we'll just note it's a reference
                exampleValue = { note: `Reference to ${exampleObj.$ref}` };
              }
  ...
  if (examples.length === 0) {
    return (
      <div className="flex justify-center items-center w-full">
        <p className="text-sm text-muted-foreground p-4 bg-accent rounded-lg">No example responses available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full">
      <Tabs defaultValue="0" value={activeTab} onValueChange={setActiveTab} className="h-full">
