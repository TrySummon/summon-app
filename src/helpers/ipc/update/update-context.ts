import { UPDATE_CHECK_CHANNEL, UPDATE_INSTALL_CHANNEL, UPDATE_STATUS_CHANNEL } from "./update-channels";
  ...
  try {
    ...
    contextBridge.exposeInMainWorld("updater", {
      ...
      onUpdateStatus: (callback: (status: string) => void) => {
        ipcRenderer.on(UPDATE_STATUS_CHANNEL, (_event: any, status: string) => callback(status));
      },
