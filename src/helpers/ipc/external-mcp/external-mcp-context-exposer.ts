...
export function exposeExternalMcpContext() {
  contextBridge.exposeInMainWorld('externalMcpApi', {
    connectExternalMcpServer: (mcpId: string) => 
      ipcRenderer.invoke(CONNECT_EXTERNAL_MCP_SERVER_CHANNEL, mcpId),
      
    stopExternalMcpServer: (mcpId: string) => 
      ipcRenderer.invoke(STOP_EXTERNAL_MCP_SERVER_CHANNEL, mcpId),
    
    onExternalMcpServersUpdated: (callback: (mcpServers: Record<string, McpServerState>) => void) => {
      const subscription = (_event: Electron.IpcRendererEvent, mcpServers: Record<string, McpServerState>) => callback(mcpServers);
      ipcRenderer.on(EXTERNAL_MCP_SERVERS_UPDATED_CHANNEL, subscription);
      
      return () => {
        ipcRenderer.removeListener(EXTERNAL_MCP_SERVERS_UPDATED_CHANNEL, subscription);
      };
    }
  });
}
